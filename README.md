...
A supervisor agent decomposes a complex request into a dependency-ordered plan, delegates each subtask to a specialized tool-using agent, and routes every deliverable through an independent reviewer before it's accepted. When the plan's confidence is low, a specialist fails repeatedly, or a review keeps failing, the system stops and escalates instead of guessing. Every decision — plan, tool call, review, escalation — is persisted, not just logged to stdout.
...
2. **Plan** — the supervisor decomposes the request into subtasks with dependencies, assigns each to a specialist, and reports a confidence score. Plans are schema-validated (unique ids, resolvable dependencies, no cycles) and retried once if invalid.
3. **Schedule & execute** — independent subtasks run in parallel; each specialist works through a bounded tool-use loop, calling only the tools it owns.
...
| API | FastAPI | ✅ |
| Persistent state | PostgreSQL — tasks, plans, subtasks, tool invocations | ✅ |
| Short-term memory | Redis | ⬜ planned |
| Long-term memory | ChromaDB | ⬜ planned |
| Async execution | Celery + Redis | 🚧 wired, exercised in the full deployment |
...
| 2 | Working memory (Redis) + long-term semantic memory (ChromaDB) | ⬜ Planned |
...
├── main.py                # FastAPI app
├── api/routes/             # tasks.py — POST /tasks, GET /tasks/{id}
...
└── workers/                # Celery app and task wrapper

tests/
├── unit/                   # schemas, registry, tools, graph routing
├── integration/            # API → graph → Postgres, live smoke test
└── fixtures/llm/           # recorded LLM responses for deterministic runs
```

`memory/`, `hitl/`, `observability/`, and `ui/` land in later phases (see [Project status](#project-status)).
